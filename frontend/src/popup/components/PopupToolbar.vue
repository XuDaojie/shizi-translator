...
const props = defineProps<{
  pinned: boolean
  /** 是否显示关闭按钮（来自 AppConfig.showCloseButton） */
...
/** 真正销毁弹窗（非 hide）；托盘进程仍在，下次翻译会 ensure 重建。 */
const closePopup = async (): Promise<void> => {
  const apis = getTauriApis()
  if (!apis) { toast.info(t('popup.error.tauriUnavailable')); return }
  try {
    await apis.getCurrentWindow().destroy()
  } catch (e) {
    toast.error(t('popup.error.closeFailed'), String(e))
  }
}
</script>
  ...
  <div class="toolbar" data-tauri-drag-region>
    ...
    <div class="toolbar-right">
      ...
      <button
        ...
        type="button"
        :title="t('popup.tooltip.close')"
        :aria-label="t('popup.tooltip.close')"
        @click="closePopup"
      >
