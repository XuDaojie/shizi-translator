...
- 高级日志系统：前后端独立日志文件（后端 `Shizi.log` / 前端 `frontend.log`），运行时等级切换（error/warn/info/debug），API Key 与翻译正文脱敏，5MB 轮转 + 启动清理 7 天，一键导出 zip（含日志/配置快照/系统信息）；OCR 路径 debug 可记识别全文与 Vision 诊断字段（永不写 Key/图 base64）。
