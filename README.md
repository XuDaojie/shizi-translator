...
**Nightly（每日滚动构建）：** 需要尝鲜主干最新改动时，可下载固定入口 [Nightly Release](https://github.com/XuDaojie/shizi-translator/releases/tag/nightly)（tag `nightly`）。安装包版本与文件名带 `nightly` 标记，形如 `Shizi_0.7.0-nightly.20260721.38b8200_x64-setup.exe`，与正式版 `Shizi_0.7.0_x64-setup.exe` 可区分；每次成功构建会清理旧资产后只保留当前包。仅供尝鲜、不保证稳定；应用内「检查更新」不会提示 Nightly（tag 非 semver，且不进入 stable/beta 通道）。也可在 Actions 中手动运行 `Nightly` workflow 补跑。
...
截图翻译始终使用设置页**当前启用**的 OCR 引擎；独立文字识别窗口默认同样回落到启用项，并支持会话级临时切换（见下方「独立文字识别」）。
...
托盘「文字识别」（或在设置中自行绑定的快捷键）打开识别窗口，支持截图框选、打开图片 / **PDF（仅第 1 页）**、读取剪贴板位图。结果仅展示识别文本与元信息，**不**自动翻译、不写入翻译历史。

- 顶栏可**临时切换** OCR 渠道（列表为全部已配置识别服务，含未启用项）；选择只作用于本窗口当前会话，**不**写 `config.json`、不改设置页互斥启用；关窗重开后回落到设置页启用项。
- `Alt+S` 截图翻译仍始终使用设置页**当前启用**的引擎，不受本窗临时选择影响。
- 多页 PDF 仅识别第 1 页；有页数时界面可提示「第 1 页 / 共 N 页」。
...
- **托盘常驻**：关闭窗口默认隐藏到托盘；托盘菜单为系统原生结构（划词翻译 / 截图翻译 / 文字识别 → 偏好设置 → 退出），右侧展示与全局快捷键同源的加速键，保存配置后热更新。
...
- **OCR 双引擎**：系统 Windows.Media.Ocr，或 OpenAI 兼容视觉模型；截图翻译与独立识别默认共用当前启用的引擎。
- **文字识别窗**：支持打开图片与 **PDF 首页**识别；可在本窗**会话级临时切换** OCR 渠道（不改设置页启用状态）；剪贴板仍为位图。
