...
- **全局快捷键**：`Alt+D` 划词复制并自动翻译；`Alt+S` 触发截图 OCR 翻译（DXGI 抓光标所在显示器整屏帧 → 自建 overlay 区域框选 → crop → Windows.Media.Ocr → 复用翻译链路）。由 `tauri-plugin-global-shortcut` 注册，逻辑集中在 `src-tauri/src/app/shortcuts.rs`。启动注册为 best-effort：单条快捷键被其他应用占用只记录到 `AppState.shortcut_conflicts`，不阻止启动；冲突列表经 `get_shortcut_conflicts` command 供设置页快捷键模块展示（保存路径仍为 all-or-nothing，失败回滚旧配置）。新增快捷键时需在 `capabilities/default.json` 同步授权。
