...
- [x] **步骤 1：增补 windows crate features**
...
- [x] **步骤 5：Commit**（与任务 7 一起提交，因 ocr_popup 此刻编译不过）
...
- [x] **步骤 1：写 overlay.rs（建窗 + 四个 command）**
...
- [x] **步骤 2：改 ocr_popup.rs（抓帧 + 建 overlay；friendly_ocr_error 改 pub）**
...
- [x] **步骤 2：人工验证（Windows，`npm run tauri dev`）**
