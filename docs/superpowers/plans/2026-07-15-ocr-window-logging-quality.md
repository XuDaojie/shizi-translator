...
- [ ] **步骤 1：编写失败的测试（扩展现有 `request_body_is_non_streaming_with_image_url` 或新测）**
...
- [ ] **步骤 3：实现函数；修复 `ocr_translation.rs`**
...
- [ ] **步骤 3：实现 meta 模块、改 encode、改 vision 使用 info**
...
- [ ] **步骤 1：定义返回类型（可放 `core/ocr/meta.rs`）**
...
- [ ] **步骤 3：unsupported 返回 `UnsupportedPlatform`**
...
- [ ] **步骤 4：测试 + Commit**（若 ocr_window 未就绪，handler 可暂 `todo` 编译不过——应同时建最小 stub）
...
- [ ] **步骤 3：修正 friendly 文案中的 Alt+O → 中性或 Alt+S**

- [ ] **步骤 4：`cargo test --lib` + Commit**
...
- [ ] **步骤 3：setup 中 `ensure_ocr_window`（与 settings 一样预创建 hidden）**
...
- [ ] **步骤 3：UI 布局（Tailwind，与 settings 风格接近）**
