...
- [x] **步骤 2：新增 show_translation_popup**
...
- [x] **步骤 3：移除 start_translation_from_input 中的 show_window**
...
- [x] **步骤 4：show_translation_error 改为唤起弹窗**
...
- [x] **步骤 2：handle_selection_translate 中先唤起弹窗**
...
- [x] **步骤 1：抽出 build_overlay 并改造 open_overlay**
...
- [x] **步骤 2：submit_capture_region 中先唤起弹窗**
...
- [x] **步骤 1：传 config 给 open_overlay**
...
- [x] **步骤 2：验证编译（任务 9 + 10 合并）**

运行：`cd src-tauri && cargo build`
预期：编译通过，无 `toggle_window` 未找到错误，无 dead_code 警告。

- [x] **步骤 3：Commit（任务 9 + 10 合并）**
...
- [x] **步骤 4：手动验证（mock provider，桌面环境）**
