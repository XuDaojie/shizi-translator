...
- [x] **步骤 1：编写失败的测试（先建 `version.rs` 仅含 tests 调用的 API 签名占位亦可，推荐 tests 与实现同文件）**
...
- [x] **步骤 1：实现 `github.rs`（纯逻辑错误映射 + 可选单测 mock JSON 解析）**
...
- [x] **步骤 2：i18n key（zh-CN / en-US 必准；其它语言可先英/中合理翻译）**
