...
- [x] **步骤 3：在文件末尾追加 utilities 与 keyframes**
...
- [x] **步骤 1：搬入 types.ts 并扩展 3 个已实现字段**
...
- [x] **步骤 2：搬入 tokens.ts（仅图标 import 改写）**
...
- [x] **步骤 1：搬入 lib/toast.ts（原样）**

把原型 `src/lib/toast.ts` 原样复制到 `frontend/src/lib/toast.ts`，无改写。

- [x] **步骤 2：搬入 stores/settings.ts 并做 5 处改造**
...
- [x] **步骤 3：核对 toast 组件引用的 toast store 路径**
...
- [x] **步骤 2：搬入 11 个原子组件 + index + types**
...
- [x] **步骤 3：SettingRow 的 wip 文案统一为「实现中」（spec §6）**
...
- [x] **步骤 4：SettingsSidebar 历史分类徽标文案同步**
...
- [x] **步骤 3：AdvancedPanel 新增「使用统计」行（已实现 collectUsage）**
...
- [x] **步骤 4：ShortcutPanel 改造（Alt+T/Alt+O 只读 + 全行「实现中」标签）**
