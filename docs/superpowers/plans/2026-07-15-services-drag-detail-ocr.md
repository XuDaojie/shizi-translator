...
- [x] **步骤 2：将选中区 button 改为非 button 可点击容器**
...
- [x] **步骤 2：手工验收（spec §4.3 / §5.5 / §6.7）**

- [x] WebView2 服务列表可拖；视觉无回归
- [x] 高级区默认折叠；卡片提示词 dirty/重置/变量 chip/反思折叠
- [x] Key 校验 / 拉模型 / 保存仍可用
- [x] OCR Tab 计数正确；无专用 OCR；默认 Windows 不可删不可关
- [x] 可添加视觉渠道；保存重开仍在
- [x] 截图 OCR 仍走 Windows（改视觉 enabled 不影响）
- [x] 翻译 Tab 零回归
