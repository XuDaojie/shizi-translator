...
- [x] **步骤 3：创建 useTranslationEvents.ts**
...
- [x] **步骤 1：创建 TranslationPopup.vue**
...
- [x] **步骤 1：创建新的 frontend/translate.html**
...
- [x] **步骤 2：修改 vite.config.ts 增 translate input**
...
- [x] **步骤 3：修改 tsconfig.json include 增 translate.html**
...
- [x] **步骤 1：端到端手动验证（spec 9.1 清单）**

运行 `npm run tauri dev`，逐项核对（不勾选直到亲眼确认）：

- [x] 手输 + Enter 触发多渠道并发流式翻译
- [x] 状态栏文案：就绪 -> 翻译中… -> 翻译完成 / 部分完成 / 翻译失败
- [x] Alt+D 划词 -> 原文区显示「来自划词」徽章
- [x] Alt+E OCR -> 原文区显示「来自 OCR」徽章
- [x] 取消：状态栏「翻译失败」，卡片追加「[已取消]」
- [x] 单个渠道失败：仅对应卡片红字，其他不受影响
- [x] 图钉：`setAlwaysOnTop` 生效，图标变蓝
- [x] 打开设置按钮：`open_settings` 触发设置窗口
- [x] OCR 按钮：`trigger_ocr_translation` 触发
- [x] 语言 combobox：搜索 / ↑↓ / Enter / Esc / 点外收
- [x] 交换：正常可交换；`auto` 时提示「自动检测不支持交换」
- [x] 检测徽章：`source=auto` -> 「检测中…」-> 显示 detectedSourceLang
- [x] 卡片折叠/展开、「展开全文」在 overflow 时出现
- [x] Token 徽章在 `usage != null` 时显示
- [x] 高度自适应：短文本紧凑、长内容超屏上限 80% + 内部滚动
- [x] app-config 变更：非翻译中即时增删卡片；翻译中延迟到 batch 完成
- [x] `take_pending_source_text` 回填原文（划词触发）
...
- [x] **步骤 2：修改 settings/main.ts 加 import**
...
- [x] **步骤 3：typecheck + 手动验证（spec 9.2 清单）**
...
- [x] 左列表按今天/昨天/本周/更早分桶
- [x] trigger 筛选栏渲染 5 个按钮（当前只 screenshot 有数据，其余切换为空列表）
- [x] 点击左侧条目切换右侧详情
- [x] `SourceCardView` / `LanguageToolbar readonly` / `ResultCardView` 三处复用样式与弹窗一致
- [x] 空态提示显示
- [x] 清空全部 Dialog 确认 -> 清空数据
- [x] 左侧 aside 独立滚动、右侧文档流随窗口滚动、sticky 筛选栏不穿透
...
- [ ] **步骤 5：Tauri 打包验证**（执行时跳过，待用户 tauri build 确认）
