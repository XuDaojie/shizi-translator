...
| 备份/恢复启用条件 | **凭证齐全即可**（不必先点「测试连接」）；测试结果以按钮旁时间戳 / toast 反馈 |
| 右上角徽标 | **已移除**（2026-08-27 对齐原型：不展示常驻连接状态） |
...
1. **云端备份 · WebDAV**（「配置」打开弹窗；无常驻连接状态徽标）  
2. **导出配置**  
3. **导入配置**
...
2. **手动同步**（立即备份 / 从云端恢复 + 上次备份时间）  
3. **自动备份** / **包含翻译历史** / **包含 API Key**

说明：2026-08-27 对齐原型，WebDAV 收进配置弹窗，本机导入导出并入同组。
