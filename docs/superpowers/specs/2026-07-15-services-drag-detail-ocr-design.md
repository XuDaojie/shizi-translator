...
- 日期：2026-07-15
- 状态：已实现（自动化验收通过；WebView2 人工点验建议发布前再走）
...
- [x] 在 WebView2 设置窗口中可拖动重排，指示线与落点正确
- [x] 侧视对比：列表行视觉与改前一致（允许 DOM 标签从 button→div 的不可见差异）
- [x] 刷新/重开后顺序仍在（保存后）
...
- [x] Tab 计数正确；无专用 OCR 入口
- [x] 默认 Windows；不可删不可关
- [x] 可添加上表视觉渠道；详情可填 Key/模型/Endpoint/识别提示词
- [x] 保存后重开数据仍在
- [x] 翻译 Tab 与拖拽修复无回归
- [x] 截图 OCR 仍走 Windows（手动或既有测试路径确认）

实现状态（2026-07-15）：代码与自动化测试已通过；WebView2 侧视拖拽 / 窄栏视觉 / 截图路径人工点验建议在发布前再走一遍。
