...
- [ ] Alt+D 划词 → `source-badge = 来自划词`
- [ ] Alt+E OCR → `source-badge = 来自 OCR`
- [ ] 取消：状态栏"翻译失败"，卡片追加"[已取消]"
- [ ] 单个渠道失败：仅对应卡片红字，其他不受影响
- [ ] 图钉：`setAlwaysOnTop` 生效，图标变蓝
- [ ] 打开设置按钮：`open_settings` command 触发
- [ ] OCR 按钮：`trigger_ocr_translation` command 触发
- [ ] 语言 combobox：搜索 / ↑↓ / Enter / Esc / 点外收
- [ ] 交换：正常状态可交换；`auto` 时提示不支持
- [ ] 检测徽章：`source=auto` → "检测中…" → 显示 `detectedSourceLang`
- [ ] 卡片折叠/展开、"展开全文"按钮在 overflow 时出现
- [ ] Token 徽章在 `usage != null` 时显示
- [ ] 高度自适应：短文本紧凑、长内容超屏时上限 80% + 内部滚动
- [ ] app-config 变更事件：非翻译中即时新增/删除/更新卡片；翻译中延迟到 batch 完成
- [ ] `take_pending_source_text` 回填原文（划词触发场景）
- [ ] Edge 环境采集 `save_edge_translate_env`
- [ ] 前端日志按 logLevel 生效（`app-config:changed` 联动）

### 9.2 历史面板验证

- [ ] 左列表按今天/昨天/本周/更早分桶
- [ ] trigger 筛选栏渲染 5 个按钮（当前只 screenshot 有数据）
- [ ] 点击左侧条目切换右侧详情
- [ ] `SourceCardView` / `LanguageToolbar readonly` / `ResultCardView` 三处复用样式与弹窗完全一致
- [ ] 空态提示 + 快捷键 kbd 显示（复刻现有）
- [ ] 清空全部 Dialog 确认 → 清空数据

### 9.3 静态检查

- [ ] `npm run typecheck` 通过（新增 popup 组件类型无 error）
- [ ] `npm run test` 通过（原有 vitest 全绿 + `useTranslationEvents` 新增单测）
- [ ] `cd src-tauri && cargo test` 通过（后端未动，应零影响）

### 9.4 打包验证

- [ ] `npm run build` 产出 `dist/settings.html` + `dist/translate.html` 两个入口
- [ ] `npm run tauri build` 生成 NSIS 安装包，主窗口能正常加载新 `translate.html`
