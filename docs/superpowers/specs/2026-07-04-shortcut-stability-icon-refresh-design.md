...
视觉上，设置页左上角目前没有明确的品牌图标，托盘图标仍是旧资源。用户已确认本轮采用“字形导向”的方案 A：深色底块上的 `文A` 双字徽记。
...
`frontend/src/settings/SettingsSidebar.vue` 顶部从纯文字“设置”改成带品牌图标的头部：

- 左侧：`文A` 徽记
- 右侧：`Shizi`
- 下方：保留简短辅助文案，说明这是应用设置页

不把图标扩散到更多位置，避免本轮范围膨胀。
...
- 与图标资源或设置页品牌展示相关的说明文档（如果已有对应条目）。

## 规格自检

- 无 TODO / 待定占位。
- 明确了为什么修 `Pressed`、为什么补 `shortcuts` 链路、为什么只迁移旧默认未自定义用户。
- 图标范围限定在“设置页左上角 + 托盘”，没有扩散到无关页面。
- 范围可由一个实现计划覆盖。
