...
pub fn provider_for_service(config: &ServiceInstanceConfig) -> Result<Box<dyn LlmProvider>, String> {
    match config.protocol.as_str() {
        "mock" => Ok(Box::new(MockLlmProvider)),
        "claude" => Ok(Box::new(ClaudeProvider::new(ClaudeConfig {
        ...
        _ => Ok(Box::new(OpenAiCompatibleProvider::new(OpenAiCompatibleConfig {
            api_key: config.api_key.clone(),
            base_url: config.endpoint.clone(),
            model: config.model.clone(),
            timeout_seconds: config.timeout_seconds as u64,
