...
fn init_app_platform() -> Result<()> {
    // Prefer PerMonitorV2. If the host process (e.g. Tauri/tao) already set process
    // DPI awareness, SetProcessDpiAwarenessContext fails with ERROR_ACCESS_DENIED /
    // 0x80070005. That is benign for embedding — do not abort Application::Start.
    //
    // (Vendored shizi patch vs upstream 884c9bb; remove when upstream tolerates this.)
    // SAFETY: FFI call into user32; no aliasing requirements.
    if let Err(err) =
        unsafe { SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2).ok() }
    {
        ...
        // HRESULT_FROM_WIN32(5) / E_ACCESSDENIED
        const E_ACCESSDENIED: windows_core::HRESULT =
